...
void DevTools::drawNodeAttributes(CCNode* node) {
	if (ImGui::Button("Deselect")) {
		node->setUserObject("input-texture-name"_spr, nullptr);
		node->setUserObject("show-input-not-changed"_spr, nullptr);
		node->setUserObject("show-texture-not-found"_spr, nullptr);
		return this->selectNode(nullptr);
	}
	ImGui::SameLine();
	if (ImGui::Button(U8STR(FEATHER_COPY " Copy Class Name"))) {
		clipboard::write(getNodeName(node) GEODE_WINDOWS(.c_str() + 6));
	}
	...
	if (auto userData = node->getUserData()) {
	...
	if (auto scale9Sprite = typeinfo_cast<CCScale9Sprite*>(node)) {
		if (auto textureProtocol = typeinfo_cast<CCTextureProtocol*>(scale9Sprite)); // try if (!(auto nonTexture = typeinfo_cast<CCTextureProtocol*>(scale9Sprite))), it wont compile
		else {
			auto spriteBatchNode = scale9Sprite->_scale9Image;
			if (auto texture = spriteBatchNode->getTexture()) {
				auto cachedFrames = CCSpriteFrameCache::sharedSpriteFrameCache()->m_pSpriteFrames;
				auto const& rect = scale9Sprite->m_spriteRect;
				for (auto const& [key, frame] : CCDictionaryExt<std::string, CCSpriteFrame*>(cachedFrames)) {
				...
				if (ImGui::DragFloat4("Rect", textureRect, 0.03f)) 
					scale9Sprite->m_spriteRect = {textureRect[0], textureRect[1], textureRect[2], textureRect[3]};

				auto capInsets = scale9Sprite->getCapInsets();
				float capInsetsArr[4] = {
					capInsets.getMinX(),
					capInsets.getMinY(),
					capInsets.size.width,
					capInsets.size.height,
				};
				if (ImGui::DragFloat4("Stretchable zone", capInsetsArr, 0.03f))
					scale9Sprite->setCapInsets({capInsetsArr[0], capInsetsArr[1], capInsetsArr[2], capInsetsArr[3]});

				ImGui::NewLine();
				auto cachedTextures = CCTextureCache::sharedTextureCache()->m_pTextures;
				for (auto const& [key, cachedTexture] : CCDictionaryExt<std::string, CCTexture2D*>(cachedTextures)) {
					if (cachedTexture == texture) {
						std::string fileName = std::filesystem::path(key).filename().string();
						if (auto newTextureStr = typeinfo_cast<CCString*>(node->getUserObject("input-texture-name"_spr))) 
							fileName = newTextureStr->getCString();
						ImGui::Text("Texture name: ");
						ImGui::SameLine();
						if (ImGui::InputText("", &fileName)) {
							node->setUserObject("input-texture-name"_spr, CCString::create(fileName));

							node->setUserObject("show-input-not-changed"_spr, CCBool::create(false));
							node->setUserObject("show-texture-not-found"_spr, CCBool::create(false));
						}

						ImGui::SameLine();
						if (ImGui::Button("Apply")) {
							if (auto newTextureStr = typeinfo_cast<CCString*>(node->getUserObject("input-texture-name"_spr))) {
								auto newTexture = CCTextureCache::get()->addImage(newTextureStr->getCString(), false);
								if (newTexture) textureProtocol->setTexture(newTexture);
								else {
									node->setUserObject("show-texture-not-found"_spr, CCBool::create(true));
								}
							} else {
								node->setUserObject("show-input-not-changed"_spr, CCBool::create(true));
							}
						}
						if (auto obj = typeinfo_cast<CCBool*>(node->getUserObject("show-texture-not-found"_spr))) {
							if (obj->getValue()) ImGui::Text("Texture not found.");
						}
						else if (auto obj = typeinfo_cast<CCBool*>(node->getUserObject("show-input-not-changed"_spr))) 
							if (obj->getValue()) ImGui::Text("Input not changed.");
	...
	if (auto textureProtocol = typeinfo_cast<CCTextureProtocol*>(node)) {
		if (auto texture = textureProtocol->getTexture()) {
			...
			ImGui::NewLine();
			auto* cachedTextures = CCTextureCache::sharedTextureCache()->m_pTextures;
			for (auto const& [key, obj] : CCDictionaryExt<std::string, CCTexture2D*>(cachedTextures)) {
				if (obj == texture) {
					std::string fileName = std::filesystem::path(key).filename().string();
					if (auto newTextureStr = typeinfo_cast<CCString*>(node->getUserObject("input-texture-name"_spr))) 
						fileName = newTextureStr->getCString();
	...
	if (auto menuItemNode = typeinfo_cast<CCMenuItem*>(node)) {
		...
		if (auto menuItemToggler = typeinfo_cast<CCMenuItemToggler*>(menuItemNode)) 
			checkbox("Toggled##toggledmenuitemtoggler", menuItemToggler, &CCMenuItemToggler::isToggled, &CCMenuItemToggler::toggle);
	...
	if (auto rawOpts = node->getLayoutOptions()) {
		ImGui::Text("Layout options: %s", getNodeName(rawOpts).c_str());
	...
	if (auto rawLayout = node->getLayout()) {
		ImGui::Text("Layout: %s", getNodeName(rawLayout).c_str());
